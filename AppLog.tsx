...
function AppLogs() {
  ...
  const [content, setContent] = useState<{ peerList: string[], datetime: string }[]>([]);
  ...
  useEffect(() => {
    if (!loaded.current) {
      ...
      MDS.init((msg) => {
        if (msg.event === 'inited') {
          streamrClient.subscribe(STREAM_ID, (content: any) => {
            setContent(prevState => [{ ...content, datetime: format(new Date(), ) }, ...prevState]);
          })
        }
      })
    }
