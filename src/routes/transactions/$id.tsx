...
function Index() {
  ...
  const isPending = data?.onChain?.confirmations && Number(data.onChain.confirmations) < 3 || true;
  const confirmations = data?.onChain?.confirmations;
      ...
      <Section className="-mt-4 mb-4 text-black lg:-mt-8 lg:mb-10 dark:text-grey80">
        ...
        {data && isPending && (
          <div className="flex items-center gap-2">
            ...
            {confirmations && (
              <div className="flex w-fit items-center gap-3 border px-3 py-1.5 text-xs lg:text-sm dark:border-gray-700 dark:bg-gray-700 dark:bg-opacity-20">
                <span className="text-black dark:text-white">Confirmations: {confirmations}</span>
              </div>
            )}
          </div>
